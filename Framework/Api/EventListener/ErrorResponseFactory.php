 ...
 *
 * @phpstan-import-type ErrorData from CicadaHttpException as CicadaExceptionData
 */
#[Package('core')]
class ErrorResponseFactory
{
    ...
    /**
     * @return array<DefaultExceptionData|CicadaExceptionData>
     */
    public function getErrorsFromException(\Throwable $exception, bool $debug = false): array
    {
        if ($exception instanceof CicadaHttpException) {
            $errors = [];
    ...
    private function getStatusCodeFromException(\Throwable $exception): int
    {
        ...
        if ($exception instanceof CicadaHttpException || $exception instanceof HttpException) {
