...
use Cicada\Core\Framework\Changelog\ChangelogFile;
use Cicada\Core\Framework\Changelog\ChangelogFileCollection;
...
class ChangelogProcessor
{
    ...
    {
        $result = shell_exec('gh release list -R cicada/cicada --exclude-drafts --exclude-pre-releases --json tagName,isLatest');
        if (!$result) {
            return null;
        }
    ...
    public function getFixCommits(string $fromRef): array
    {
        ...
        foreach ($refs as $ref) {
            ...
            if ($body && preg_match_all('/' . ChangelogParser::FIXES_REGEX . '/i', $body, $matches)) {
                ...
                if ($author && !$this->isCicadaOrgMember($author['login'])) {
                    $fix['author'] = $author;
                }
    ...
    {
        $result = shell_exec(\sprintf('gh pr view https://github.com/cicada/cicada/pull/%s --json author', escapeshellarg(ltrim($issueId, '#'))));
    ...
    private function isCicadaOrgMember(string $login): bool
    {
        if ($this->users === null) {
            $this->users = [];
            $result = shell_exec('gh api --paginate -H "Accept: application/vnd.github+json" -H "X-GitHub-Api-Version: 2022-11-28" /orgs/cicada/members');
            if ($result) {
