...
class EntityCompilerPass implements CompilerPassInterface
{
    ...
    private function collectDefinitions(ContainerBuilder $container): void
    {
        $entityNameMap = [];
        $repositoryNameMap = [];
        $services = $container->findTaggedServiceIds('cicada.entity.definition');
        ...
        foreach ($ids as $serviceId) {
            ...
            if (!\is_subclass_of($class, EntityDefinition::class)) {
                throw DependencyInjectionException::taggedServiceHasWrongType($serviceId, 'cicada.entity.definition', EntityDefinition::class);
            }
            $instance = new $class();
    ...
    {
        $servicesIds = array_keys($container->findTaggedServiceIds('cicada.field_serializer'));
    ...
    {
        $servicesIds = array_keys($container->findTaggedServiceIds('cicada.field_resolver'));
    ...
    {
        $servicesIds = array_keys($container->findTaggedServiceIds('cicada.field_accessor_builder'));
