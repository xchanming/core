...
class PluginService
{
    ...
    public function refreshPlugins(Context $cicadaContext, IOInterface $composerIO): ExceptionCollection
    {
        ...
        $installedPlugins = $this->getPlugins(new Criteria(), $cicadaContext);
        ...
        foreach ($pluginsFromFileSystem as $pluginFromFileSystem) {
            ...
            $pluginData['translations'] = $this->getTranslations($cicadaContext, $extra);
        ...
        if ($plugins !== []) {
            foreach ($plugins as $plugin) {
                try {
                    $this->pluginRepo->upsert([$plugin], $cicadaContext);
                } catch (CicadaHttpException $exception) {
        ...
        if (\count($deletePluginIds) !== 0) {
            ...
            $this->pluginRepo->delete($deletePlugins, $cicadaContext);
        }
    ...
    private function getTranslations(Context $context, array $extra): array
    {
         ...
         * @example payload
         * {
         *     "cicada-plugin-class":"Swag\\MyDemoData\\MyDemoData",
         *     "label":{
