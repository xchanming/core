...
use Cicada\Core\Framework\Plugin\Requirement\Exception\ComposerNameMissingException;
use Cicada\Core\Framework\Plugin\Requirement\Exception\ConflictingPackageException;
...
class RequirementsValidator
{
    ...
    private Composer $cicadaProjectComposer;
    ...
    public function validateRequirements(PluginEntity $plugin, Context $context, string $method): void
    {
        ...
        $this->cicadaProjectComposer = $this->getComposer($this->projectDir);
    ...
    private function checkComposerDependencies(
        ...
        foreach ($packages as $package) {
            // Ignore Cicada plugins. They are checked separately in `validateInstalledPlugins`
    ...
    {
        $packages = $this->cicadaProjectComposer->getRepositoryManager()->getLocalRepository()->getPackages();
