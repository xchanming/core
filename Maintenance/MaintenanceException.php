...
use Cicada\Core\Maintenance\System\Exception\JwtCertificateGenerationException;
...
class MaintenanceException extends HttpException
{
    ...
    public static function dbVersionMismatch(
        ...
        if (!Feature::isActive('v6.7.0.0')) {
            return new DatabaseSetupException(
                \sprintf(
                    'Your database server is running %s %s, but Cicada 6 requires at least MySQL %s OR MariaDB %s',
                    $dbKind,
                    $actualVersion,
